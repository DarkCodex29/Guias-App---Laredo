...
class CsvService {
  ...
  Future<String> generateCsv({
    ...
    try {
      // Crear nombre de archivo según formato SUNAT
      final String csvFileName = '$rucEmisor-09-$serieCorrelativo.csv';

      // En móvil, guardamos directamente en /storage/emulated/0/Download/Guias
      final directory = Directory('/storage/emulated/0/Download/Guias');
      final String filePath = '${directory.path}/$csvFileName';

      LoggerService.info('Usando directorio de descargas: ${directory.path}');

      // Crear directorio si no existe
      if (!await directory.exists()) {
        await directory.create(recursive: true);
        LoggerService.info('Directorio Guías creado en: ${directory.path}');
      } else {
        LoggerService.info('Directorio Guías ya existe en: ${directory.path}');
      }
      ...
      if (data['items'] != null && (data['items'] as List).isNotEmpty) {
        ...
        for (final item in data['items']) {
          ...
          if (item['cuartel'] != null &&
              item['cuartel'].toString().isNotEmpty) {
      ...
      LoggerService.error('Error al generar CSV: $e');
      rethrow;
    }
  }
}
